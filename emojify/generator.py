...
import emojify.util.emoji
import emojify.util.files
import emojify.util.text
...
class EmojipastaGenerator:
    ...
    def generate_emojipasta(self, text):
        blocks = emojify.util.text.split_into_blocks(text)
        new_blocks = []
    ...
    def _generate_emojis_from(self, block):
        trimmed_block = emojify.util.text.trim_nonalphabetical_characters(block)
...
def _get_emoji_mappings():
    ...
    if _EMOJI_MAPPINGS is None:
        with io.open(emojify.util.files.PATH_TO_MAPPINGS_FILE, "r", encoding="utf-8") as mappings_file:
